        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $selected_values = isset($relationshipData) ? (!isset($options->model_key_name)?$relationshipData->belongsToMany($options->model, $options->pivot_table):$relationshipData->belongsToMany($options->model, $options->pivot_table, $options->model_key_name, $options->relationship_key_name))->get()->map(function ($item, $key) use ($options) {
            			return $item->{$options->label};
            		})->all() : array();
                @endphp
            ...
            @else
                ...
                >
                        ...
                        @php
                            $selected_values = isset($dataTypeContent) ? (!isset($options->model_key_name)?$dataTypeContent->belongsToMany($options->model, $options->pivot_table):$dataTypeContent->belongsToMany($options->model, $options->pivot_table, $options->model_key_name, $options->relationship_key_name))->get()->map(function ($item, $key) use ($options) {
