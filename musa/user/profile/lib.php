...
function profile_save_data($usernew) {
    ...
    // save company-user into tb company_user
    save_company_user($usernew);
}
...
// save user into table company_user
function save_company_user($data) {
    global $DB;

    $userid = $data->id;
    if ($companycode = $data->profile_field_companycode) {
        if (!$company = $DB->get_record('company',
          array('company_code' => $companycode), 'id')) {
            return false;       //empty company, do noting
        }
        if($recordExist = $DB->record_exists('company_user',
            array('company_id' => $company->id, 'user_id' => $userid))) {
            if ($recordExist) return false;         //exist relation, do noting
        }
        $obj = new stdClass;
        $obj->company_id = $company->id;
        $obj->user_id = $userid;
        $DB->insert_record('company_user', $obj);
        return true;
    }
}
