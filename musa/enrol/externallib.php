 ...
 * 简历
 */
class core_resume_external extends external_api
{
    public static function get_user_resume_parameters()
    {
        return new external_function_parameters(
            array(
                'userid' => new external_value(PARAM_INT, 'User enrolment ID')
            )
        );
    }

    public static function get_user_resume_returns()
    {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'resumeid' => new external_value(PARAM_INT),
                    'companyid' => new external_value(PARAM_INT),
                    'name' => new external_value(PARAM_RAW),
                    'career' => new external_value(PARAM_RAW),
                    'startwork' => new external_value(PARAM_INT),
                    'topeducation' => new external_value(PARAM_RAW),
                    'jobtype' => new external_value(PARAM_RAW),
                    'intro' => new external_value(PARAM_RAW),
                    'source' => new external_value(PARAM_RAW),
                    'companys' => new external_multiple_structure(
                        new external_single_structure(
                            array(
                                'companyname' => new external_value(PARAM_RAW),
                                'industry' => new external_value(PARAM_RAW),
                                'jobtitle' => new external_value(PARAM_RAW),
                                'jobtype' => new external_value(PARAM_RAW),
                                'jobstart' => new external_value(PARAM_RAW),
                                'jobend' => new external_value(PARAM_RAW),
                                'salary' => new external_value(PARAM_RAW),
                                'jobdesc' => new external_value(PARAM_RAW),
                            )
                        )),
                    'createdate' => new external_value(PARAM_RAW),
                )
            )
        );
    }

    public static function get_user_resume($uid)
    {
        global $DB;
        $params = self::validate_parameters(self::get_user_resume_parameters(), array('userid' => $uid));
        $sql = "SELECT * from {resume} r LEFT JOIN {resume_basic} rb on
         r.id = rb.resumeid where r.userid = $uid and source='normal'";
        $resumes = $DB->get_records_sql($sql);
        foreach ($resumes as $key => $resume) {
            $companys = $DB->get_records('resume_company',
                array('resumeid' => $resume->resumeid));
            $resumes[$key]->companys = array_values($companys);
            // var_dump($resumes[$key]->companys);exit;
