...
//该第三方企业正在为本企业招聘的职位数量
        ...
        foreach ($list as &$v) {
            ...
            $v->allJobCount = Recruit::whereIn('id', Entrust::where('third_party_id',$v->id)->whereNotIn('status', [0,-2])->pluck('company_job_recruit_id')->toArray())->sum('need_num');
            $v->ourJobCount = $_all_count;
            $v->currentRecruitCount = Recruit::whereIn('id', Entrust::where('third_party_id',$v->id)->where('company_id',$company->id)->whereIn('status', [1])->pluck('company_job_recruit_id')->toArray())->sum('need_num');
