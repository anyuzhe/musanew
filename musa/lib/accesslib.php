...
class context_helper extends context {
    ...
    private static function init_levels() {
        ...
        self::$alllevels = array(
            ...
            CONTEXT_COMPANY     => 'context_company_user',
        );
...
class context_company_user extends context {
    /**
     * Please use context_user::instance($userid) if you need the instance of context.
     * Alternatively if you know only the context id use context::instance_by_id($contextid)
     *
     ...
     * @return context_user context instance
     */
    public static function instance($userid, $strictness = null) {
        global $DB, $USER;

        if ($context = context::cache_get(CONTEXT_COMPANY, $userid)) {
            return $context;
        }
        if (!$record = $DB->get_record('context', array('contextlevel' => CONTEXT_COMPANY, 'instanceid' => $userid))) {
            if ($companyuser = $DB->get_record('company_user', array('userid' => $userid), 'id')) {
                $record = context::insert_context_record(CONTEXT_COMPANY, $userid, '/'.SYSCONTEXTID, 0);
            }
            else {
                if ($USER->auth = 'manual' ) {
                    $record = context::insert_context_record(CONTEXT_COMPANY, $userid, '/'.SYSCONTEXTID, 0);
        ...
        if ($record) {
            $context = new context_company_user($record);
            context::cache_add($context);
            return $context;
        }
    ...
    protected static function create_level_instances() {
        ...
        $sql = "SELECT ".CONTEXT_COMPANY.", u.id
                  FROM {user} u
                 WHERE u.deleted = 0
                       AND NOT EXISTS (SELECT 'x'
                                         FROM {context} cx
                                        WHERE u.id = cx.instanceid AND cx.contextlevel=".CONTEXT_COMPANY.")";
        $contextdata = $DB->get_recordset_sql($sql);
        foreach ($contextdata as $context) {
